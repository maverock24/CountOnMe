...
export default function TabOneScreen() {
  ...
  const handleNativeStatusUpdate = useCallback(
    (status: NativeRecordingStatusWithMetering) => {
  ...
  const handleSliderChange = useCallback((value: number) => {
    ...
    if (Platform.OS === 'web') {
      try {
        ...
        console.error('Web: Error saving sensitivity:', e);
      }
    } else {
      storeItem(key, value.toString()).catch((err) =>
        console.error('Native: Error saving sensitivity:', err)
      );
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (Platform.OS === 'web') {
      if (isLoopActiveRef.current && audioLevel > 0) {
        ...
        const webThreshold = Math.round(
          maxThreshold - (sensitivitySetting / 100) * (maxThreshold - minThreshold)
        );
      ...
      if (nativeRecorder?.isRecording && audioLevel !== -160) {
        const minDbThreshold = -70;
        const maxDbThreshold = -20;
        const nativeThreshold = Math.round(
  ...
  const stopListeningWeb = async () => {
    ...
    if (audioContextRef.current && audioContextRef.current.state !== 'closed') {
      await audioContextRef.current
        .close()
        .catch((e) => console.error('Web: Error closing context:', e));
    }
  ...
  const analyzeAudioWeb = () => {
    ...
    if (isMountedRef.current) {
      setAudioLevel(peakDeviation);
    }
    if (isLoopActiveRef.current && isMountedRef.current) {
      animationFrameRef.current = requestAnimationFrame(analyzeAudioWeb);
    } else {
      animationFrameRef.current = null;
    }
  };
  ...
  else if ((currentPlatform as any) !== 'web' && audioLevel > -159.9)
    levelDisplayText = `${t('mic_level')}: ${audioLevel.toFixed(1)} dB`;
              ...
              <View style={styles.buttonContainerReps}>
                {repititions.map((rep, index) => (
                  <TimerButton
                    ...
                    style={{ marginHorizontal: 5, width: 77 }}
                  />
                ))}
              </View>
